...
## Commands                                                                                                                                                       
  ...
  Onboarding Wizard — /onboarding is a 5-step client-side wizard collecting artist name/phone (step 1), home region (step 2), social links (step 3), bio/photo (step 4), and connecting a personal Gmail/Outlook account (step 5, required — see Personal Email Sending). The profile and zone are saved when moving from step 4 to step 5 (not at the very end), since step 5 involves a full-page redirect to Google/Microsoft that would otherwise lose in-progress data. Photo upload is non-blocking — failures don't stop onboarding. Step 5 has no upfront skip; a short-lived cookie tags the next OAuth attempt as belonging to onboarding, cleared by the local "Continue without connecting for now" fallback (shown only after a failed attempt is observed on this page). Routing a failed attempt back to this step from the Artist Profile page is a separate, not-yet-shipped piece of this feature — a failure currently lands on Artist Profile with its existing generic error banner instead. The middleware checks for artist_profiles.display_name and routes incomplete users back to /onboarding.                 
