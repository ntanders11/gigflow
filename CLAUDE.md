...
## Commands                                                                                                                                                       
  ...
  Discover Venues — components/discover/DiscoverView.tsx auto-searches on load using the user's home zone. GET /api/venues/discover geocodes the city server-side (Geoapify Geocoding first, Nominatim with countrycodes=us as fallback) then queries Geoapify Places (bar/pub/nightclub/brewery/winery) for venues, falling back to OpenStreetMap Overpass if Geoapify is unavailable. Both are free, no billing required. Search is deliberately scoped to small, informal venues — no concert halls, theaters, universities, or event venues, which book through a different channel than a bar or club does. Venues already in the user's pipeline are silently excluded from results (no greyed-out duplicates). The page displays required "Powered by Geoapify" + OpenStreetMap attribution per Geoapify's free-tier terms.
  ...
  - GEOAPIFY_API_KEY — venue discovery (Places search) and server-side geocoding. Free tier, no billing required, no card on file. (GOOGLE_PLACES_API_KEY is defined but unused — Google was tried as the primary provider and got fully working, including billing + a 200/day hard quota cap on both Places and Geocoding APIs, but was reverted in favor of Geoapify: comparable accuracy once properly filtered, zero cost/billing risk. The Google code path is fully preserved in git history if ever wanted back — the quota caps are still live on that Cloud project even though unused.)
