...
4. Write, as two sequential steps: first, set `featured_by_artist_rank` (or `featured_by_venue_rank`) to `null` on **every** row owned by the caller that currently has a rank set — including ones staying in the new selection, not just ones being dropped. Second, write ranks 1..n to the given ids in array order. Clearing before reassigning (rather than only clearing the dropped ones) avoids a spurious unique-constraint conflict when a request reorders two already-featured picks — e.g. swapping ranks 1 and 2 would otherwise try to write rank 2 onto the row still holding rank 1 mid-update. Since `createServiceClient()` issues plain PostgREST calls rather than raw SQL, these two steps are not atomic; if the second step fails after the first succeeds, the caller's featured picks end up cleared rather than partially-wrong, and the UI should show an error and let them re-submit.
